...
#define SCHED_HEAP_MAX       1024
#define SCHED_PATH_MAX       512
#define MAX_TASKS_PER_BOX    8
#define SCHED_MAX_ENTRIES    32
#define LAST_DIR_NONE        4
#define BEST_HASH_SIZE       2048
...
static int task_push_is_clear(
    ...
    uint8 prog[3], uint8 cur_boxes[][2], map_input_t *map)
{
    uint8 bx;
    uint8 by;
...
static uint32 best_make_key(uint8 car, uint8 p0, uint8 p1, uint8 p2, uint8 ld)
{
    return (((((uint32)car * 9U + p0) * 9U + p1) * 9U + p2) * 5U + ld) + 1U;
}

static uint16 best_get(uint8 car, uint8 p0, uint8 p1, uint8 p2, uint8 ld)
{
    uint32 k = best_make_key(car, p0, p1, p2, ld);
...
static void best_set(uint8 car, uint8 p0, uint8 p1, uint8 p2,
                     uint8 ld, uint16 val)
{
    uint32 k = best_make_key(car, p0, p1, p2, ld);
    uint16 h = (uint16)(k % BEST_HASH_SIZE);
    ...
    schedule_entry_t *out_schedule, uint8 *out_len)
{
    ...
    *out_len = 0;
    ...
    best_set(car_idx, 0, 0, 0, LAST_DIR_NONE, 0);
    ...
    while(sched_heap_size > 0) {
        ...
        uint8 prog[3] = {cur.p0, cur.p1, cur.p2};
        uint8 done = 1;

        if((uint16)cur.cost > best_get(cur.car_idx, cur.p0, cur.p1, cur.p2, cur.last_dir))
            continue;
...

        {
            ...
            for(uint8 bid = 0; bid < box_count; bid++) {
                ...
                old = best_get(new_car_idx, new_p[0], new_p[1], new_p[2], task->push_dir);
                if((uint16)new_cost < old) {
                    uint16 path_idx;
                    if(sched_path_count >= SCHED_PATH_MAX) continue;
                    path_idx = sched_path_count++;

                    best_set(new_car_idx, new_p[0], new_p[1], new_p[2],
                             task->push_dir, (uint16)new_cost);
    ...
    action_t *actions, uint8 *act_count)
{
    ...
    uint8 car_cy = car_y;
    uint8 cur_boxes[MAX_BOXES][2];
    uint8 box_done[MAX_BOXES] = {0U, 0U, 0U};
    (void)pp;
