...
#define CONTROL_DEBUG_ENABLE   (3U)

// 调试子模式：1=单电机开环，2=三电机开环，3=位置保持，4=航向保持，5=向右平移，6=左前轮方向切换。
#define CONTROL_VERIFY_MODE    (5U)
...
static void app_target_init(void)
{
    ...
    uart_write_string(TEST_UART, "#MCU_BOOT id=7100893 stage=CONTROL_DEBUG mode=" STRINGIFY(CONTROL_VERIFY_MODE) " screen=IPS200\r\n");
#elif CONTROL_DEBUG_ENABLE == 2U
    uart_write_string(TEST_UART, "#POINT_TEST_PREBOOT id=7100893 safe=1 menu=UART8_D16_D17 display=OFF telemetry=3s cpr=4096 odom_scale=0.915 pos_type=0x12 car_valid=ON\r\n");
#elif CONTROL_DEBUG_ENABLE == 3U
    uart_write_string(TEST_UART, "#TRACK_CAL_PREBOOT id=7100893 safe=1 menu=UART8_D16_D17 display=OFF manual=ON auto=QUICK12+STANDARD60+FULL180\r\n");
#else
    uart_write_string(TEST_UART, "#UART8_D16_D17_RX id=7100893 safe=1 control=GRID_STEP_MENU display=OFF cpr=4096 odom_scale=0.915 pos_type=0x12 car_valid=ON tx=IRQ_QUEUE\r\n");
#endif
...
static void control_debug_open_loop_task(void)
{
    ...
    ips200_show_string(0, 16, "ID:7100893 PWM:");
...
static void control_debug_show_closed_loop(void)
{
    ...
    if(CONTROL_VERIFY_MODE == 3U)
    {
        ...
        ips200_show_string(0, 16, "ID:7100893 PWM CAP 1800");
    ...
    ips200_show_string(0, 16, "ID:7100893 ZERO=BOOT YAW");
...
int main(void)
{
    ...
    {
        uart_write_string(TEST_UART, "#WIRELESS_UART8_READY id=7100893 baud=115200 tx=D16 rx=D17 display=OFF\r\n");
    }
    else
    {
        uart_write_string(TEST_UART, "#WIRELESS_UART8_INIT_FAIL id=7100893\r\n");
    }
#endif
