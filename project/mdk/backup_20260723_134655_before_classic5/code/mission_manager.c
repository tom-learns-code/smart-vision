                                        ...
                                        mission_exec_context_t *out)
{
    ...
    out->follower.strict_position = (uint8)(interaction ||
        out->object_near || out->goal_near);
    ...
    out->merge_eligible = (uint8)(!interaction && !out->object_near &&
        !out->goal_near && !out->transition &&
        (out->follower.gear == ACTION_GEAR_OPEN_FAST ||
         out->follower.gear == ACTION_GEAR_SINGLE_WALL_FAST ||
         out->follower.gear == ACTION_GEAR_CORRIDOR_FAST));
    return 1U;
...
static uint8 mission_plan_environment_current(void)
{
    ...
    if(!mission_expected_map_valid || !vision_link_is_online()) return 0U;
...
static uint8 mission_prepare_push_step(const action_t *action,
                                       uint8 *from_x, uint8 *from_y,
                                       uint8 *to_x, uint8 *to_y)
{
    vision_link_map_t latest;
    ...
    if(push->push_dir > DIR_RIGHT || p >= push->n_steps) return 0U;
    dx = (int)DIR_DX[push->push_dir];
    dy = (int)DIR_DY[push->push_dir];
    car_from_x = (int)push->car_target_x + dx * p;
    car_from_y = (int)push->car_target_y + dy * p;
    box_from_x = (int)push->box_start_x + dx * p;
    box_from_y = (int)push->box_start_y + dy * p;
    box_to_x = box_from_x + dx;
    box_to_y = box_from_y + dy;

    if(!mission_cell_in_map(&latest, car_from_x, car_from_y) ||
       !mission_cell_in_map(&latest, box_from_x, box_from_y) ||
       !mission_cell_in_map(&latest, box_to_x, box_to_y) ||
    ...
    detonate = (uint8)(is_bomb &&
        mission_wall_at(&latest, (uint8)box_to_x, (uint8)box_to_y));
    if(mission_wall_at(&latest, (uint8)car_from_x, (uint8)car_from_y) ||
       mission_cell_list_has(latest.bombs, latest.bomb_count,
                             (uint8)car_from_x, (uint8)car_from_y) ||
       mission_cell_list_has(latest.boxes, latest.box_count,
                             (uint8)car_from_x, (uint8)car_from_y) ||
       (is_bomb ?
          !mission_cell_list_has(latest.bombs, latest.bomb_count,
    ...
    mission_push_expected_map = mission_expected_map;
    if(is_bomb)
    {
        for(i = 0U; i < mission_push_expected_map.bomb_count; i++)
        {
            if(mission_push_expected_map.bombs[i].gx == (int8)box_from_x &&
               mission_push_expected_map.bombs[i].gy == (int8)box_from_y)
            {
                uint8 j;
                if(moved) return 0U;
                moved = 1U;
                ...
                {
                    for(j = i; j + 1U <
                         mission_push_expected_map.bomb_count; j++)
                        mission_push_expected_map.bombs[j] =
                            mission_push_expected_map.bombs[j + 1U];
                    mission_push_expected_map.bomb_count--;
                }
                else
                {
                    mission_push_expected_map.bombs[i].gx =
                        (int8)box_to_x;
                    mission_push_expected_map.bombs[i].gy =
                        (int8)box_to_y;
                }
                break;
            }
            ...
            for(clear_y = box_to_y - 1; clear_y <= box_to_y + 1; clear_y++)
                for(clear_x = box_to_x - 1; clear_x <= box_to_x + 1;
                    ...
                    if(clear_x >= 0 && clear_y >= 0 &&
                       clear_x < (int)mission_push_expected_map.width &&
                       clear_y < (int)mission_push_expected_map.height)
                        mission_clear_wall_at(&mission_push_expected_map,
                                              (uint8)clear_x,
                                              (uint8)clear_y);
        }
    }
    ...
    {
        for(i = 0U; i < mission_push_expected_map.box_count; i++)
        {
            if(mission_push_expected_map.boxes[i].gx == (int8)box_from_x &&
               mission_push_expected_map.boxes[i].gy == (int8)box_from_y)
            {
                if(moved) return 0U;
                mission_push_expected_map.boxes[i].gx = (int8)box_to_x;
                mission_push_expected_map.boxes[i].gy = (int8)box_to_y;
                moved = 1U;
            }
        }
    }
    ...
    *from_x = (uint8)car_from_x;
    *from_y = (uint8)car_from_y;
    *to_x = (uint8)box_from_x;
    *to_y = (uint8)box_from_y;
    mission_status.push_step = 1U;
    mission_status.push_is_bomb = is_bomb;
    mission_status.box_from_x = (uint8)box_from_x;
    mission_status.box_from_y = (uint8)box_from_y;
    mission_status.box_to_x = (uint8)box_to_x;
    mission_status.box_to_y = (uint8)box_to_y;
    mission_status.push_box_on_goal = (uint8)(!is_bomb &&
        box_to_x == (int)push->box_target_x &&
        box_to_y == (int)push->box_target_y &&
        mission_cell_list_has(mission_expected_map.goals,
...
static mission_result_t mission_start_current_step(uint8 allow_push)
{
    ...
    if(action->type == ACTION_FREE_MOVE)
    {
           ...
           context.merge_eligible)
        {
            ...
            if(segment_cells >= 3U)
            {
                ...
                mission_terminal_context.follower.strong_reanchor = 1U;
            ...
            else
            {
                ...
                context.follower.terminal_node = 1U;
                context.follower.strong_reanchor = 1U;
    ...
    {
        if(!mission_prepare_push_step(action, &from_x, &from_y, &to_x, &to_y))
        {
            mission_hard_stop(MISSION_FAULT,
