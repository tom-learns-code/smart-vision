...
static void pc_console_report_rx(const char *source, uint8 value)
{
    ...
    else if(value >= 0x21U && value <= 0x7EU)
    {
        snprintf(text, sizeof(text),
                 "UART8_RX id=7100967 tx=D16 rx=D17 src=%s hex=%02X ascii='%c'\r\n",
                 source,
    ...
    snprintf(text, sizeof(text),
             "UART8_RX id=7100967 tx=D16 rx=D17 src=%s hex=%02X ascii=%s\r\n",
             source,
...
{
    pc_console_write("\r\n=== SMART CAR MENU id=7100967 state=SAFE_IDLE_UART8_RX ===\r\n");
...
static void pc_console_print_fp_camera_menu(void)
{
    ...
    snprintf(text, sizeof(text),
             "\r\n=== FP2 CAMERA MENU id=7100967 state=%s online=%u session=%u ready=%u ===\r\n",
...
static void pc_console_print_planner_menu(void)
{
    char text[96];
    snprintf(text, sizeof(text),
             "\r\n=== PLANNER MENU id=7100967 state=GRID_PLAN_MOTORS_LOCKED solver=%s ===\r\n",
...
static void pc_console_print_run_menu(void)
{
    ...
    snprintf(text, sizeof(text),
             "\r\n=== RUN MENU id=7100967 mode=GRID_STEP_M4 profile=%s speed=%d ===\r\n",
...
static void pc_console_print_match_status(void)
{
    ...
    snprintf(text, sizeof(text),
             "MATCH id=7100967 state=%s gate=%s wait=%u reject=%u/%u recovery=%s fault=%s kind=%s profile=%s armed=%u run=%u remote=%u ready=0x%02X/0x%02X skip_base=%u round=%u/%u done=%u success=%u elapsed=%lums/%lums event=%lu\r\n",
...
static void pc_console_print_match_runtime(const match_status_t *match)
{
    ...
    snprintf(text, sizeof(text),
             "MATCH_RT id=7100967 seq=%lu state=%s motor=%s point=%s mission=%s a=%u wp=%u/%u phase=%s dir=%s/%d speed=%u/%u progress=%ld remain=%ld assist=%u/0x%02X/%ums yaw=%ld/%ld err=%ld corr=%ld gyro=%ld wheel_t=(%d,%d,%d) pid_t=(%d,%d,%d) enc=(%d,%d,%d) pwm=(%d,%d,%d) txq=%u drop=%lu\r\n",
    ...
    snprintf(text, sizeof(text),
             "MATCH_VIS id=7100967 seq=%lu online=%u pose=%u age=%lums car=(%d.%d,%d.%d) theta=%d.%d map=%lu valid=%u size=%ux%u wall=%u elem=%u/%u/%u candidate=%s before=%lu req=%lu state_ms=%lu\r\n",
             (unsigned long)g_match_runtime_sequence,
...
static void pc_console_print_match_menu(void)
{
    ...
    match_manager_get_status(&match);
    snprintf(text, sizeof(text),
             "\r\n=== COMPLETE MATCH id=7100967 state=%s preset=%s count=%u exit=FORWARD/%umm ===\r\n",
             match_state_name(match.state),
             match_preset_name(match.preset_index),
...
static void pc_console_print_point_main_menu(void)
{
    ...
    point_test_get_snapshot(&point);
    snprintf(text, sizeof(text),
             "\r\n=== POINT TEST MENU id=7100967 state=%s origin=%u ===\r\n",
             point_test_state_name(point.state),
...
static void pc_console_print_track_main_menu(void)
{
    ...
    snprintf(text, sizeof(text),
             "\r\n=== TRACK CAL MENU id=7100967 state=%s session=%lu record=%lu ===\r\n",
...
static uint8 pc_console_print_plan_summary(void)
{
    ...
    snprintf(text, sizeof(text),
             "PLAN_SUMMARY id=7100967 valid=%u current=%u status=%s world=%s solver=%s map_ver=%lu gen=%lu solve_ms=%lu\r\n",
...
static void pc_console_print_mission_status(void)
{
    ...
    mission_manager_get_status(&mission);
    snprintf(text, sizeof(text),
             "MISSION id=7100967 state=%s result=%s armed=%u auto=%u action=%u/%u sub=%u/%u step=%u/%u type=%u move=(%u,%u)->(%u,%u) gen=%lu plan_current=%u event=%lu telemetry=%s\r\n",
             mission_state_name(mission.state),
...
static void pc_console_print_run_telemetry(const mission_status_t *mission)
{
    ...
    snprintf(text, sizeof(text),
             "RUN_TELEM id=7100967 seq=%lu ms=%lu state=%s motor=%s plan=%u action=%u wp=%u/%u phase=%s seg=%u/%u/%u node=(%u,%u) speed=%d/%d hold=%lums rebase=%lu nav=%s blind=%u/%u pose=%s car=(%s,%s) age=%lums target=(%d,%d) delta=(%d,%d) dist=%u vtheta=%s ref=%s used=%s elapsed=%lums progress=%lums\r\n",
...
static void pc_console_print_point_status(uint8 detailed)
{
    ...
    snprintf(text, sizeof(text),
             "POINT_STATUS id=7100967 state=%s kind=%s fault=%s motor=%s origin=%u vis0=%u vis=%u age=%lums dir=%s/%d cells=%u speed=%u/%u brake=%umm sensor=%s rot=%udeg/%s/%s progress=%s remain=%s elapsed=%lums settle=%lums assist=%u/%u mask=0x%02X left=%ums stall=%lums/%ldmm event=%lu\r\n",
             point_test_state_name(point.state),
             point_test_kind_name(point.kind),
             point_test_fault_name(point.fault),
             device_init_flag ? "LOCKED" : "ENABLED",
...
static void pc_console_print_track_record(const track_cal_record_t *record)
{
    ...
    snprintf(text, sizeof(text),
             "CAL_REC id=7100967 schema=1 seq=%lu session=%lu profile=%s step=%u/%u valid=%u result=%s point=%s\r\n",
    ...
    const track_rotation_record_t *record)
{
    ...
    snprintf(text, sizeof(text),
             "ROT_REC id=7100967 schema=1 seq=%lu session=%lu mode=%s sequence=%s step=%u/%u valid=%u result=%s point=%s\r\n",
             (unsigned long)record->sequence,
             (unsigned long)record->session,
...
static void pc_console_print_track_status(uint8 include_record)
{
    ...
    track_calibration_get_snapshot(&track);
    snprintf(text, sizeof(text),
             "CAL_STATUS id=7100967 state=%s motor=%s session=%lu profile=%s selected=%s step=%u/%u valid=%u invalid=%u paused=%u result=%s point=%s txq=%u txdrop=%lu\r\n",
             track_calibration_state_name(track.state),
             device_init_flag ? "LOCKED" : "ENABLED",
             (unsigned long)track.session,
...
static void pc_console_print_status(void)
{
    ...
    snprintf(text, sizeof(text),
             "STATUS id=7100967 motor=%s mission=%s online=%u pose_valid=%u age=%lu rx=%lu pos=%lu map=%lu ver=%lu req=%lu err=%lu/%lu drop=%lu txq=%u txpeak=%u txdrop=%lu car=(%d.%d,%d.%d) theta=%d.%d\r\n",
             device_init_flag ? "LOCKED" : "ENABLED",
             mission_state_name(mission.state),
             (unsigned int)vision_link_is_online(),
...
static void pc_console_poll_roi_camera_report(void)
{
    ...
    snprintf(text, sizeof(text),
             "FP2_STATUS mcu_id=7100967 fp_id=%lu online=%u uart=MCU1_B12_B13/CAM12 state=%s session=%u ready=%u count=%u/%u pending=%u:%u/%u retry=%u status=%s rx=%lu proto=%lu tx=%lu match=%lu stale=%lu age=%lums timeout=%lu crc=%lu fmt=%lu drop=%lu\r\n",
...
void pc_console_init(uint8 app_mode)
{
    ...
    if(g_app_mode == 3U)
    {
        pc_console_write("\r\n#MCU_BOOT id=7100967 stage=ROT_CHECK_5MS safe=1 debug=3 display=OFF\r\n");
        snprintf(text, sizeof(text),
                 "UART8_CONFIG id=7100967 baud=115200 tx=D16 rx=D17 rx_drop=%lu tx=IRQ_QUEUE tx_size=%u tx_isr_budget=%u telemetry=ON/3s display=OFF mode3=TRACK_CAL fp_uart12=UART1_B12_B13/FP2 auto=GRID1+GRID2+GRID3+LENGTH36+STANDARD60+FULL180 rotation=AUTO16_45_90_135_180+OUT_BACK+HAND start_gate=VIS_POS check=5ms approach=20deg/40 brake=7deg hold=1.5deg+3dps/200ms translation_brake=T100_F50_R30_B47_L20+T120_F40_R20_B37_L10+T150_F55_R35_B52_L25 startup=TRANSLATION_WHEELS_ONLY/300ms/min3000-2100-2200 stall=5mm/1000ms gate=100mm wheel=%umm cpr=%u odom_scale_x1000=%u center=100mm cell=200mm buzzer=B11_5MS_ISR\r\n",
    ...
    {
        pc_console_write("\r\n#MCU_BOOT id=7100967 stage=POINT_TEST_M4_FP2_READY safe=1 debug=2 display=OFF\r\n");
        snprintf(text, sizeof(text),
                 "UART8_CONFIG id=7100967 baud=115200 tx=D16 rx=D17 rx_drop=%lu tx=IRQ_QUEUE tx_size=%u tx_isr_budget=%u telemetry=ON/3s sensor_frame=COMPACT display=OFF pos_type=0x12 fp_uart12=UART1_B12_B13/FP2 wheel=%umm cpr=%u odom_scale_x1000=%u center=100mm cell=200mm brake=T100_F50_R30_B47_L20+T120_F40_R20_B37_L10+T150_F55_R35_B52_L25 stall=5mm/1000ms startup=TRANSLATION_WHEELS_ONLY/300ms solver=GRID4+PLAN_CHECK heading=QUANTIZED_ON_ARM+IMU_LOCK m4=CAR_VALID+Z_STABLE6_200MS+ENC_IMU_MOVE+HOLD500+DRAIN1000+STABLE6_200MS+VIS_CHECK100 push=AUTO_POSE_ONLY+N_MAP_VERIFY buzzer=B11_5MS_ISR\r\n",
                 (unsigned long)g_uart8_rx_drops,
                 (unsigned int)PC_CONSOLE_TX_RING_SIZE,
                 (unsigned int)PC_CONSOLE_TX_ISR_BUDGET,
                 (unsigned int)(motion_get_wheel_diameter_mm() + 0.5f),
    ...
    {
        pc_console_write("\r\n#MCU_BOOT id=7100967 stage=UART_BUTTON_HYBRID safe=1 debug=0 display=OFF\r\n");
        snprintf(text, sizeof(text),
                 "UART8_CONFIG id=7100967 baud=115200 tx=D16 rx=D17 rx_drop=%lu tx=IRQ_QUEUE tx_size=%u tx_isr_budget=%u telemetry=BASE_EXIT125MS+MATCH_RUNTIME250MS+EVENT display=OFF fp_uart12=UART1_B12_B13/FP2 fp_report=EVENT+S pos_type=0x12 cpr=%u odom_scale_x1000=%u mode=RACE presets=%u/max6 rounds=RUN_STRATEGY_SPEED_ALGORITHM base=ENC_IMU_FORWARD450 map=REQUEST_NONEMPTY_THEN_VIS_STABLE solver=ALGO1_CLASSIC+ALGO2_IMAGE_NO_BOMB+ALGO3_BOMB_IMAGE return=CLEAR_DIRECT+MAP_BFS next=AUTO_THREE_MAP buttons=C13_HOLD_STOP+C14_HOLD_ARM_START physical_start=REQUIRED behavior=STABLE_BASELINE direction=PLAN_CARDINAL+OPEN_VIS_CROSS+NODE_TERMINAL+OBJECT8_GRID1 vision=POS_STABLE+OBJECT8_STRICT20_30_ATTEMPT3+SUSPECT6_200MS+WALL_SNAP_STRICT heading=ROUND_FIXED_POST_MAP+IMU_LOCK+REPLAN_RESTORE push=ONE_GRID+STRICT_OBJECT8+POST_REANCHOR_ROLLING20MM+RAMP300_D150 buzzer=B11_5MS_ISR_QUEUE\r\n",
                 (unsigned long)g_uart8_rx_drops,
                 (unsigned int)PC_CONSOLE_TX_RING_SIZE,
                 (unsigned int)PC_CONSOLE_TX_ISR_BUDGET,
                 (unsigned int)(motion_get_wheel_cpr() + 0.5f),
