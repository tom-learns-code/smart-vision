...
#define VISION_LINK_POS_MAX_BOXES      (4U)
#define VISION_LINK_POS_MAX_BOMBS      (4U)
#define VISION_LINK_FULL_PAYLOAD_LEN   (60U)
...
static void vision_link_parse_full_map(void)
{
    ...
    if(vision_link_xor(rx_payload, VISION_LINK_FULL_PAYLOAD_LEN - 1U) != rx_payload[VISION_LINK_FULL_PAYLOAD_LEN - 1U])
    {
    ...
    vision_link_copy_cells(g_vis.boxes, VISION_LINK_MAX_BOXES, g_vis.box_count, 27U);

    count = rx_payload[35];
    g_vis.goal_count = (count > VISION_LINK_MAX_GOALS) ? VISION_LINK_MAX_GOALS : count;
    vision_link_copy_cells(g_vis.goals, VISION_LINK_MAX_GOALS, g_vis.goal_count, 36U);

    count = rx_payload[44];
    g_vis.bomb_count = (count > VISION_LINK_MAX_BOMBS) ? VISION_LINK_MAX_BOMBS : count;
    vision_link_copy_cells(g_vis.bombs, VISION_LINK_MAX_BOMBS, g_vis.bomb_count, 45U);
    ...
    g_vis.last_type = VISION_LINK_TYPE_FULL_MAP;
