...
from core.map_analysis import load_map_from_file, scan_map, assign_random_ids, get_walls_in_3x3
...
def create_objects_from_map(map_data, offset_x=OFFSET_X, offset_y=OFFSET_Y):
    ...
    # 获取ID分配
    box_label_ids = map_data.get('box_label_ids', {})
    goal_label_ids = map_data.get('goal_label_ids', {})

    boxes = []
    for i, (gx, gy) in enumerate(boxes_list):
        wx, wy = grid_to_world(gx, gy)
        ...
        box.label_id = box_label_ids.get(i, -1)
        box.discovered_id = False
        boxes.append(box)
    ...
    targets = []
    goals_sorted = sorted(goals_set, key=lambda p: (p[1], p[0]))  # 行优先，对齐c_solver dot_pos
    for i, (gx, gy) in enumerate(goals_sorted):
        wx, wy = grid_to_world(gx, gy)
        tgt = GameFactory.create_target()
        tgt.pos = Vector2(wx + offset_x, wy + offset_y)
        tgt.label_id = goal_label_ids.get(i, -1)
        tgt.discovered_id = False
        targets.append(tgt)
...
def draw_targets(screen, targets, occupied_goals=None):
    ...
    for t in targets:
        ...
        if g in occupied_goals:
            ...
            # 未推达：空心正方形（和车/箱/弹大小一致40px）
            s = OBJECT_SIZE
            pygame.draw.rect(screen, COLOR_TARGET,
                             (cx - s // 2, cy - s // 2, s, s), 2)
        # ID标签
        if hasattr(t, 'label_id') and t.label_id is not None and t.label_id >= 0:
            try:
                if t.discovered_id:
                    text = str(t.label_id)
                    color = (255, 255, 255)
...
def show_save_confirm(screen, action_count):
    """阻塞式确认弹窗，返回 True=保存, False=放弃"""
    font_title = FontManager.get_font(28)
    font_btn = FontManager.get_font(22)
    font_hint = FontManager.get_font(16)
    ...
    buttons = [
        {"text": "保存 (Y)", "rect": pygame.Rect(px + 50, py + 110, 120, 40), "value": True},
        {"text": "放弃 (N)", "rect": pygame.Rect(px + 230, py + 110, 120, 40), "value": False},
    ]
    ...
    while True:
        ...
        # 弹窗背景
        pygame.draw.rect(screen, (50, 55, 65), (px, py, pw, ph), border_radius=12)
        pygame.draw.rect(screen, (120, 130, 150), (px, py, pw, ph), 2, border_radius=12)

        # 标题
        title = font_title.render("录制完成，是否保存？", True, (255, 255, 255))
        screen.blit(title, (px + (pw - title.get_width()) // 2, py + 25))

        # 操作数提示
        hint = font_hint.render(f"共 {action_count} 条操作记录", True, (180, 180, 190))
        screen.blit(hint, (px + (pw - hint.get_width()) // 2, py + 65))

        # 按钮
        mouse_pos = pygame.mouse.get_pos()
        for btn in buttons:
            hover = btn["rect"].collidepoint(mouse_pos)
            bg = (70, 140, 90) if hover else (55, 110, 70) if btn["value"] else (160, 70, 70)
            if not btn["value"]:
                bg = (140, 70, 70) if hover else (110, 55, 55)
            pygame.draw.rect(screen, bg, btn["rect"], border_radius=8)
            text = font_btn.render(btn["text"], True, (255, 255, 255))
...
def draw_status(screen, pf, auto_mode, boxes=None, targets=None, recording=False, rec_idx=0, computing=False):
    ...
    if computing:
        ...
        if boxes and targets:
            disc_boxes = sum(1 for b in boxes if b.discovered_id)
            disc_targets = sum(1 for t in targets if t.discovered_id)
            lines.append(f"识别: 箱{disc_boxes}/{len(boxes)}  目标{disc_targets}/{len(targets)}")
        lines += [
            "",
            "T=运行  R=求解  Z=软重置",
            "SPACE=硬重置  ←→=切图  WASD=手动",
...
def main():
    ...
    # 动态导入求解器
    from run_solver import solve_with_c_solver, solve_phase2_dijkstra

    # 全局 P1 结果（供后续 P2 调用）
    g_p1_res = None

    def load_map_only(filepath_arg=None):
        """仅加载地图+分配ID，不求解。返回 (raw, info)"""
        fp = filepath_arg if filepath_arg else filepath
        raw = load_map_from_file(fp)
        info = scan_map(raw)

        n_boxes = info['counts']['boxes']
        n_goals = info['counts']['goals']
        box_label_ids, goal_label_ids = assign_random_ids(n_boxes, n_goals)
        info['box_label_ids'] = box_label_ids
        info['goal_label_ids'] = goal_label_ids
        raw['box_label_ids'] = box_label_ids
        raw['goal_label_ids'] = goal_label_ids

        if n_boxes > 0:
            box_ids_str = ', '.join(f'箱{k}={v}' for k, v in sorted(box_label_ids.items()))
            goal_ids_str = ', '.join(f'目标{k}={v}' for k, v in sorted(goal_label_ids.items()))
            print(f"[ID] {box_ids_str}  |  {goal_ids_str}")

        return raw, info

    def do_solve(raw, screen=None):
        """执行求解，返回 result。如果 screen 不为 None，先在屏幕上显示提示"""
        nonlocal g_p1_res
        if screen is not None:
            _show_computing(screen)
        result = solve_with_c_solver(raw)
        g_p1_res = result.get('p1_res')
        return result

    def _show_computing(screen):
        """在屏幕上显示'正在计算路径...'"""
        screen.fill(COLOR_BG)
        draw_grid(screen)
        font = FontManager.get_font(36)
        text = font.render("正在计算路径...", True, (255, 255, 100))
        screen.blit(text, (SCREEN_WIDTH // 2 - text.get_width() // 2,
                           SCREEN_HEIGHT // 2 - text.get_height() // 2))
        font_small = FontManager.get_font(18)
        hint = font_small.render("（c_solver 联合A*搜索，请耐心等待）", True, (180, 180, 180))
        screen.blit(hint, (SCREEN_WIDTH // 2 - hint.get_width() // 2,
                           SCREEN_HEIGHT // 2 + 30))
        pygame.display.flip()

    # 启动：只加载地图，不求解
    raw, info = load_map_only()
    result = {'success': False, 'actions': [], 'error': '尚未求解（按 T 开始）'}
    map_basename = os.path.splitext(os.path.basename(filepath))[0]
    ...
    computing = False       # 正在计算路径
    p1_actions_backup = []  # 软重置用：P1 actions 备份（不含追加的P2）
    ...
    while running:
        ...
        if st.action_idx != prev_action_idx:
            # === 处理刚完成的 action ===
            prev_action = actions_list[prev_action_idx] if 0 <= prev_action_idx < len(actions_list) else None

            # observe: 标记物体为已发现
            if prev_action and prev_action['type'] == 'observe':
                meta = prev_action.get('observe_meta', {})
                obj_type = meta.get('object_type', '')
                obj_idx = meta.get('object_index', -1)
                look_dir = meta.get('direction', '')
                # 计算被观察的格子坐标
                obs_tc, obs_tr = prev_action['target']
                dir_dc, dir_dr = {'UP': (0, -1), 'DOWN': (0, 1), 'LEFT': (-1, 0), 'RIGHT': (1, 0)}.get(look_dir, (0, 0))
                look_c, look_r = obs_tc + dir_dc, obs_tr + dir_dr

                if obj_type in ('box', 'both') and 0 <= obj_idx < len(boxes):
                    boxes[obj_idx].discovered_id = True
                    lid = boxes[obj_idx].label_id
                    print(f"[模拟] 发现 箱{obj_idx} = {lid}")
                if obj_type in ('goal', 'both'):
                    # 查找被观察格上的目标点
                    for ti, t in enumerate(targets):
                        tg = world_to_grid(t.pos.x - OFFSET_X, t.pos.y - OFFSET_Y)
                        if tg == (look_c, look_r):
                            t.discovered_id = True
                            print(f"[模拟] 发现 目标{ti} = {t.label_id}")
                            break
            ...
            # 已到达目标的箱子已从物理中移除, 重叠分离必须跳过它们,
            # 否则其残留坐标会把后续经过同一目标点的车/箱子顶开 (幽灵碰撞)
            for _ in range(3):
                for i, box in enumerate(boxes):
                    if i not in completed_boxes:
                        _separate(car, box)
        ...
        # === 检测 end_of_phase1: pf 停在 IDLE 等待 P2 ===
        if pf.state == FollowerState.IDLE and not pf.is_done():
            cur_act = actions_list[pf.idx] if pf.idx < len(actions_list) else None
            if cur_act and cur_act['type'] == 'end_of_phase1':
                print("[模拟] P1 执行完毕，收集已知ID...")
                known_box_ids = []
                known_target_ids = []
                for bi, box in enumerate(boxes):
                    if box.discovered_id and box.label_id >= 0:
                        known_box_ids.append(box.label_id)
                    else:
                        known_box_ids.append(-1)
                for ti, t in enumerate(targets):
                    if t.discovered_id and t.label_id >= 0:
                        known_target_ids.append(t.label_id)
                    else:
                        known_target_ids.append(-1)

                print(f"[模拟] 已知箱ID: {[f'箱{k}={v}' for k,v in enumerate(known_box_ids)]}")
                print(f"[模拟] 已知目标ID: {[f'目标{k}={v}' for k,v in enumerate(known_target_ids)]}")

                # 调用 P2
                p2_result = solve_phase2_dijkstra(g_p1_res, known_box_ids, known_target_ids, raw)
                if p2_result['success'] and p2_result['actions']:
                    result['actions'].extend(p2_result['actions'])
                    print(f"[模拟] P2 求解成功, 追加 {len(p2_result['actions'])} 条 Action")
                else:
                    print(f"[模拟] P2 求解失败: {p2_result.get('error', '未知')}")

                # 跳过 end_of_phase1，进入 P2 动作
                pf.resume_after_phase1()
                prev_action_idx = pf.idx  # 强制触发 action 切换处理
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_ESCAPE:
                    ...
                    raw, info = load_map_only()
                    result.clear()
                    result.update(do_solve(raw, screen))
                    computing = False
                    car, boxes, bombs, targets, walls, boundary_walls = reset_objects()
                    pf.reset()
                    completed_boxes.clear()
                    ...
                    if result['success'] and result['actions']:
                        ...
                        p1_actions_backup = list(result['actions'])  # 软重置用
                    ...
                    if recording:
                        ...
                        if actions:
                            if show_save_confirm(screen, len(actions)):
                                recording_count += 1
                                ts = datetime.now().strftime('%m%d_%H%M%S')
                                fname = os.path.join(base, f'recorded_{map_basename}_{ts}.json')
                ...
                # WASD 手动控制
                    ...
                    # 软重置：保留ID和P1计算结果，仅恢复物体初始位置
                    print("[控制] 软重置（保留ID + P1）")
                    car, boxes, bombs, targets, walls, boundary_walls = reset_objects()
                    pf.reset()
                    completed_boxes.clear()
                    occupied_goals.clear()
                    detonated_bombs.clear()
                    all_walls_original = set(info['walls'])
                    result['actions'] = list(p1_actions_backup)  # 恢复纯P1
                    if result['actions']:
                        pf.load_actions(result['actions'])  # 必须用同一引用，保证P2追加后pf可见
                    auto_mode = False
                    manual_mode = False
            ...
            if g in goals_set and g not in occupied_goals:
                ...
                for t in targets:
                    ...
                    if tg == g:
                        target_on_goal = t
                        break
                if target_on_goal and box.label_id >= 0 and target_on_goal.label_id >= 0:
                    if box.label_id != target_on_goal.label_id:
                        # ID不匹配 → 箱子停在目标格上但不消失
                        box.velocity = Vector2(0, 0)
                        continue
                completed_boxes.add(i)
                ...
                act = actions[st.action_idx] if st.action_idx < len(actions) else None
                if act and act['type'] == 'push_box' and act['push_meta']['box_id'] == i:
                    pf.force_advance()
                print(f"[模拟] 箱{i}(ID={box.label_id}) 到达目标 {g}(ID={target_on_goal.label_id if target_on_goal else '?'})")
        ...
        draw_status(screen, pf, auto_mode, boxes, targets, recording, recording_count, computing)

        # 未求解时的提示
        if not computing and not result.get('actions') and result.get('error') == '尚未求解（按 T 开始）':
            font = FontManager.get_font(28)
            text = font.render("按 R 求解，T 运行", True, (255, 255, 100))
