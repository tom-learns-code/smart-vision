"""
模拟器入口 (DEBUG版) — 算法求解 + 可视化回放 + 帧级调试日志

用法:
    python run_sim_debug.py [地图文件名]
...
DEBUG_LOG_INTERVAL = 3
_debug_log_file = None
_debug_log_start_time = 0.0


def _start_debug_log(base_dir):
    global _debug_log_file, _debug_log_start_time
    ts = time.strftime('%Y%m%d_%H%M%S')
    log_path = os.path.join(base_dir, f'run_sim_debug_{ts}.log')
    _debug_log_file = open(log_path, 'w', encoding='utf-8')
    _debug_log_start_time = time.time()
    print(f'[DEBUG] 日志: {log_path}')
...
def _log_actions(actions):
    if _debug_log_file is None:
        return
    _debug_log_file.write('=== Action 序列 ===\n')
    for i, a in enumerate(actions):
        t = a['type']
        if t == 'free_move':
            wps = a.get('waypoints', [])
            _debug_log_file.write(f'  {i:3d}. FREE_MOVE -> {a["target"]} wps={wps}\n')
        elif t == 'push_box':
            m = a['push_meta']
            _debug_log_file.write(f'  {i:3d}. PUSH_BOX[{m["box_id"]}] {m["push_dir"]} {m["box_start"]}->{m["box_target"]}\n')
        elif t == 'push_bomb':
            m = a['push_meta']
            _debug_log_file.write(f'  {i:3d}. PUSH_BOMB[{m["bomb_id"]}] {m["push_dir"]} {m["bomb_start"]}->{m["bomb_target"]} wall={m["wall_target"]}\n')
        elif t == 'wait':
            _debug_log_file.write(f'  {i:3d}. WAIT ({a.get("reason","?")})\n')
    _debug_log_file.write('\n')
    _debug_log_file.flush()


def _debug_log_frame(frame_count, pf, car, boxes, bombs, completed_boxes,
                      occupied_goals, actions, auto_mode, off_x, off_y):
    global _debug_log_file, _debug_log_start_time
    if _debug_log_file is None:
        return
    ...
    t = time.time() - _debug_log_start_time
    st = pf.get_status()
    cx = car.pos.x - off_x
    cy = car.pos.y - off_y
    cg = (int(cx) // CELL_SIZE, int(cy) // CELL_SIZE)
    car_spd = math.hypot(car.velocity.x, car.velocity.y)

    act = actions[st.action_idx] if st.action_idx < len(actions) else None
    atype = act['type'] if act else '-'
    atarget = str(act.get('target', '?')) if act else '?'

    boxes_info = []
    for i, box in enumerate(boxes):
        bx = box.pos.x - off_x; by = box.pos.y - off_y
        bg = (int(bx) // 50, int(by) // 50)
        spd = math.hypot(box.velocity.x, box.velocity.y)
        done = i in completed_boxes
        boxes_info.append(f"b{i}{'✓' if done else ''}:{bg} spd={spd:.0f}")

    bombs_info = []
    for i, b in enumerate(bombs):
        if b.is_exploded:
            bombs_info.append(f"B{i}💥")
            continue
        bx = b.pos.x - off_x; by = b.pos.y - off_y
        bg = (int(bx) // 50, int(by) // 50)
        spd = math.hypot(b.velocity.x, b.velocity.y)
        bombs_info.append(f"B{i}:{bg} spd={spd:.0f}")

    push_phase = st.push_phase.name if st.push_phase else '-'

    line = (f"f{frame_count:5d} t={t:6.2f}s [{st.action_idx:2d}/{st.total}] {st.state.name:7s} {push_phase:8s} "
            f"{atype:9s} →{atarget:6s} | car:{cg} v={car_spd:5.1f} | "
            f"{' '.join(boxes_info)} | {' '.join(bombs_info)}")
...
def main():
    ...
    while running:
        ...
        # 调试日志
        _debug_log_frame(frame_count, pf, car, boxes, bombs, completed_boxes,
                         occupied_goals, actions, auto_mode, OFFSET_X, OFFSET_Y)
