...
from .action_converter import convert_p1_actions, convert_p2_actions
...
# DTCM: 堆比较热路径，q_f/q_g每节点比较都访问。3000x2B=6KB
q_f = [0] * MAX_NODES             # int16_t[MAX_NODES]  DTCM 6KB
q_g = [0] * MAX_NODES             # int16_t[MAX_NODES]  DTCM 6KB
relaxed_dist_b = [[[9999] * MAX_C for _ in range(MAX_R)] for __ in range(3)]  # OCRAM 1.1KB
...
# DTCM: 哈希表热点 (8192x4+8192x2=48KB)，每节点encode+insert必查
hash_keys = [0] * HASH_SIZE       # uint32_t[HASH_SIZE]  DTCM 32KB
hash_gen = [0] * HASH_SIZE        # uint16_t[HASH_SIZE]  DTCM 16KB
current_hash_gen = 0              # uint16_t              DTCM

q = [Node() for _ in range(MAX_NODES)]   # struct Node[MAX_NODES]  OCRAM ~90KB (3000x30B)
heap = [0] * MAX_NODES                    # int16_t[MAX_NODES]     DTCM 6KB
heap_size = 0                             # int16_t               DTCM
node_cnt = 0                              # int16_t               DTCM
...
dot_cnt = 0                      # int16_t
bfs_dist_x = [[[9999] * MAX_C for _ in range(MAX_R)] for __ in range(3)]   # OCRAM 1.1KB: 启发式get_h_p1查表
bfs_dist_dot = [[[9999] * MAX_C for _ in range(MAX_R)] for __ in range(3)] # OCRAM 1.1KB: P2启发式查表
...
g_best_p1_len = 0                        # int
g_best_p1_states = [SimState() for _ in range(MAX_STEPS)]  # P1最优解对应的状态序列
g_best_bombs = [[0, 0] for _ in range(3)]  # int16_t[3][2]
g_best_p2_acts = [0] * MAX_STEPS         # int16_t[MAX_STEPS]
g_best_p2_len = 0                        # int
g_best_p2_states = [SimState() for _ in range(MAX_STEPS)]  # P2最优解对应的状态序列
...
# OCRAM: P2阶段4D访问标记 12x16x12x16=36864项x2B=72KB，仅在P2搜索时大量使用
p2_vis_flat = None  # 在reset中重建  OCRAM 72KB
...
# DTCM: A*热路径每节点扩展都查，单次访问查8种mask，8x12x16=1536B，放DTCM
...
def reset_globals():
    ...
    global g_best_p1_states, g_best_p2_states
    ...
    g_best_p1_len = 0
    g_best_p1_states = [SimState() for _ in range(MAX_STEPS)]
    g_best_bombs = [[0, 0] for _ in range(3)]
    g_best_p2_acts = [0] * MAX_STEPS
    g_best_p2_len = 0
    g_best_p2_states = [SimState() for _ in range(MAX_STEPS)]
...
# P2 4D标记预计算偏移表: base_off[rr][rc] = rr*3072 + rc*192
_NC = MAX_C          # 16
_NRC = MAX_R * MAX_C  # 192
_base_off = [[rr * _NRC * _NC + rc * _NRC for rc in range(_NC)] for rr in range(MAX_R)]


def _rebuild_p2_vis():
    """重建展平的1D p2_vis数组 [36864] uint16_t"""
    global p2_vis_flat
    p2_vis_flat = [0] * (MAX_R * MAX_C * MAX_R * MAX_C)  # 12*16*12*16 = 36864


def _p2_vis_get(rr, rc, br, bc):
    """p2_vis[rr][rc][br][bc] 展平索引读取"""
    return p2_vis_flat[_base_off[rr][rc] + br * _NC + bc]


def _p2_vis_set(rr, rc, br, bc, val):
    """p2_vis[rr][rc][br][bc] 展平索引写入"""
    p2_vis_flat[_base_off[rr][rc] + br * _NC + bc] = val
...
def _observation_requirements(req_b: int, req_t: int) -> Tuple[int, int]:
    """Return the per-kind observation quota.

    TRACK_NUM=4 is the contest N-2 strategy: leave exactly one box and one
    target unseen, then infer that final pair.  The old port only applied this
    rule to the 3+3 case, which made smaller maps over-observe.
    """
    if TRACK_NUM == 4:
        return max(0, req_b - 1), max(0, req_t - 1)
    if TRACK_NUM == 5 and req_b == 3 and req_t == 3:
        return 2, 3
    return req_b, req_t


def _observations_satisfied(obs_b_mask: int, obs_t_mask: int,
                            req_b: int, req_t: int) -> bool:
    need_b, need_t = _observation_requirements(req_b, req_t)
    cb = count_bits(obs_b_mask)
    ct = count_bits(obs_t_mask)
    if TRACK_NUM == 5 and req_b == 3 and req_t == 3:
        return (cb >= 2 and ct >= 3) or (cb >= 3 and ct >= 2)
    return cb >= need_b and ct >= need_t


def _limit_observation_bits(current_mask: int, candidate_mask: int,
                            required_count: int) -> int:
    """Select deterministic unseen bits without exceeding a quota."""
    room = required_count - count_bits(current_mask)
    if room <= 0:
        return 0
    selected = 0
    unseen = candidate_mask & ~current_mask
    for bit in range(3):
        flag = 1 << bit
        if unseen & flag:
            selected |= flag
...
def _try_free_look(cur: SimState) -> int:
    ...
    current_look = 0
    need_b, need_t = _observation_requirements(init_bx_cnt, init_dot_cnt)
    for i in range(4):
        if count_bits(obs_b) >= need_b and count_bits(obs_t) >= need_t:
            break
        d = p_dir[i]
        nr = cur.r + dr[d]
        nc = cur.c + dc[d]
        ...
        saw = False
        for k in range(3):
            if (count_bits(obs_b) < need_b
                    and cur.bx[k] != 255 and cur.bx[k] == npos
                    and not (obs_b & (1 << k))):
                obs_b |= (1 << k)
                saw = True
        for k in range(3):
            if (count_bits(obs_t) < need_t
                    and dot_pos[k] != 255 and dot_pos[k] == npos
                    and not (obs_t & (1 << k))):
                obs_t |= (1 << k)
                saw = True
        if saw:
...
def _dfs_tsp_0bomb(u: int, current_cost: int, cur_b: int, cur_t: int,
    ...
    if _observations_satisfied(cur_b, cur_t, req_b, req_t):
    ...
    for v in range(1, len(tsp_pois)):
        ...
        need_b, need_t = _observation_requirements(req_b, req_t)
        add_b = _limit_observation_bits(cur_b, tsp_pois[v].b_mask, need_b)
        add_t = _limit_observation_bits(cur_t, tsp_pois[v].t_mask, need_t)
        if add_b or add_t:
            req_looks = []
            for d in range(4):
...
def solve_with_bombs(bombs, out_p1_steps: list, final_st: SimState,
    ...
    while heap_size > 0 and node_cnt < active_max_nodes - 10:
        ...
        # [STRUCT1] A*循环内提前终止: f值已超当前最优解时, 后续节点不可能更优
        if current_best_metric < 999999.0 and q_f[curr_idx] >= current_best_metric:
            break
        ...
        # C: 第908-909行 — 机器人可达性 vis_id (改用位掩码)
        current_reach_id += 1
        # 位掩码vis_rows替代reach_vis: 每行一个uint16_t, 12行=192位
        vis_rows = [0] * MAX_R

        # C: 第911-928行 — BFS计算从机器人当前位置到所有可达格的距离
        fast_q_bfs = [0] * 256
        head, tail = 0, 0
        start_enc = (curr.r << 4) | curr.c
        fast_q_bfs[tail] = start_enc
        tail += 1
        vis_rows[curr.r] |= (1 << curr.c)
        ...
        while head < tail:
            ...
            r = val >> 4
            c = val & 15
            cur_dist = global_dist[r][c] + 1

            # 使用预计算邻接表加速: 每个格最多4个邻居
            # 上
            if r > 0:
                nr, nc = r - 1, c
                if not cur_wall[nr][nc]:
                    if not (solid_mask[nr] & (1 << nc)) and not (vis_rows[nr] & (1 << nc)):
                        vis_rows[nr] |= (1 << nc)
                        global_dist[nr][nc] = cur_dist
        ...
        # [P2] 位掩码→reach_vis同步（下游代码依赖reach_vis检查可达性）
        for _rr in range(MAX_R):
            _row = vis_rows[_rr]
            if _row:
                for _cc in range(MAX_C):
                    if _row & (1 << _cc):
                        reach_vis[_rr][_cc] = current_reach_id
        ...
        for item_idx in range(6):
            ...
            for i_dir in range(4):
                ...
                # [ROUND_C] 状态复用池: 直接用q[node_cnt]预分配的State, 避免list()分配
                if node_cnt >= active_max_nodes:
                    continue
                next_s = q[node_cnt].s
                next_s.r = nr; next_s.c = nc
                next_s.mask = curr.mask; next_s.nudges = curr.nudges
                # 逐值拷贝b/bx（复用已有列表，避免list(curr.b)分配）
                for _ci in range(3):
                    next_s.b[_ci] = curr.b[_ci]
                    next_s.bx[_ci] = curr.bx[_ci]
                action_penalty = 0
        ...
        while h < t:
            ...
            for d in range(4):
                ...
                if not fast_wall[prev.mask][nr][nc] and p1_dist_vis[nr][nc] != p1_dist_vis_id:
                    # walk_r/walk_c是推箱/弹的站位，允许终点被物体占据
                    if nr == walk_r and nc == walk_c:
                        block = False
                    else:
                        block = False
                        for k in range(3):
    ...
    if skip_flag == 0:
        if init_b_cnt == 0:
            ...
            for i_tsp in range(tsp_best_len):
                ...
                # 执行观察动作
                need_b, need_t = _observation_requirements(init_bx_cnt, init_dot_cnt)
                add_b = _limit_observation_bits(
                    obs_b, tsp_pois[next_node].b_mask, need_b)
                add_t = _limit_observation_bits(
                    obs_t, tsp_pois[next_node].t_mask, need_t)
            ...
            while True:
                req_b = init_bx_cnt
                req_t = init_dot_cnt
                if _observations_satisfied(obs_b, obs_t, req_b, req_t):
                    break
                ...
                best_path = [0] * 512
                best_path_len_val = 0
                max_quota_b, max_quota_t = _observation_requirements(req_b, req_t)
                ...
                for i_state in range(p1_len + 1):
                    ...
                    for k in range(3):
                        ...
                        if (st.bx[k] != 255 and not (obs_b & (1 << k))
                                and cur_cb < max_quota_b):
                            check_pts[pts_cnt][0] = GET_R(st.bx[k])
                            check_pts[pts_cnt][1] = GET_C(st.bx[k])
                            check_type[pts_cnt] = 0
                            pts_cnt += 1
                        if (dot_pos[k] != 255 and not (obs_t & (1 << k))
                                and cur_ct < max_quota_t):
...
def run_phase1(raw_map, p1_res: PhaseResult, mode: int, skip_flag: int) -> bool:
    ...
    # [FIX2] 补炸弹四邻可推入墙格：凸包相交在关键点共线(退化)时会漏掉
    #        与炸弹同行/同列、可一步直推进入的墙格(如map1(1)的(5,5))。
    #        每炸弹最多补4格且去重，只增不减，对非退化图无扰动(重复格被滤掉)。
    _pool_seen = {(p[0], p[1]) for p in pool}
    _adj_dr = (-1, 1, 0, 0)
    _adj_dc = (0, 0, -1, 1)
    for _bi in range(init_b_cnt):
        _br, _bc = init_b[_bi][0], init_b[_bi][1]
        for _k in range(4):
            _nr, _nc = _br + _adj_dr[_k], _bc + _adj_dc[_k]      # 推入墙格
            _or, _oc = _br - _adj_dr[_k], _bc - _adj_dc[_k]      # 车站立(推动)反向格
            if not (1 <= _nr <= MAX_R - 2 and 1 <= _nc <= MAX_C - 2):
                continue
            if original_map[_nr][_nc] != '#':
                continue
            # 车必须能站在炸弹反向格才能把它往墙里推
            if not (0 <= _or < MAX_R and 0 <= _oc < MAX_C) or original_map[_or][_oc] == '#':
                continue
            if (_nr, _nc) not in _pool_seen:
                pool.append([_nr, _nc])
                _pool_seen.add((_nr, _nc))

    pool_size = len(pool)

    # [STRUCT2] pool按关键性排序: 离推箱/目标越近的墙先试, best_metric更快收敛
    _wall_score = [0] * pool_size
    for _wi, _w in enumerate(pool):
        _wr, _wc = _w[0], _w[1]
        _min_bx = min(abs(_wr - init_bx[_bi][0]) + abs(_wc - init_bx[_bi][1])
                      for _bi in range(init_bx_cnt)) if init_bx_cnt > 0 else 0
        _min_dt = min(abs(_wr - init_dot[_di][0]) + abs(_wc - init_dot[_di][1])
                      for _di in range(init_dot_cnt)) if init_dot_cnt > 0 else 0
        _min_b = min(abs(_wr - init_b[_bi][0]) + abs(_wc - init_b[_bi][1])
                     for _bi in range(init_b_cnt)) if init_b_cnt > 0 else 0
        _wall_score[_wi] = _min_bx + _min_dt + _min_b * 3  # 炸弹近的墙更重要
    # 同步排序pool和_score
    _sorted = sorted(zip(_wall_score, pool), key=lambda x: x[0])
    pool = [p for _, p in _sorted]
    del _wall_score, _sorted
                ...
                for k in range(start_k, lim_k):
                    ...
                    if metric >= 0.0:
                        ...
                        if metric < global_best_metric:
                            ...
                            # 同步保存P1状态序列（供action_converter使用）
                            for si in range(best_p1_steps + 1):
                                g_best_p1_states[si].r = p1_states[si].r
                                g_best_p1_states[si].c = p1_states[si].c
                                g_best_p1_states[si].mask = p1_states[si].mask
                                for _ci in range(3):
                                    g_best_p1_states[si].b[_ci] = p1_states[si].b[_ci]
                                    g_best_p1_states[si].bx[_ci] = p1_states[si].bx[_ci]
    ...
    # === 转换为结构化 action（供中间层消费） ===
    structured, obs_box_indices, obs_target_indices = convert_p1_actions(
        g_best_p1_acts, g_best_p1_states, best_p1_steps,
        init_b_cnt, init_bx_cnt, dot_pos, fast_wall,
        p1_dist_vis, global_dist, global_parent, global_action,
    )
    p1_res.structured_actions = structured
    p1_res.observed_box_indices = obs_box_indices
    p1_res.observed_target_indices = obs_target_indices
...
def run_phase2(box_ids, target_ids, p2_res: PhaseResult) -> bool:
    ...
    for i in range(init_bx_cnt):
        for j in range(init_dot_cnt):
            if box_ids[i] == target_ids[j] and j not in used_targets:
                cur_assign[i] = j
                used_targets.add(j)
                break
        if cur_assign[i] < 0:
    ...
    # C: 第1800-1902行 — 按实际箱数枚举有效推箱排列
    if init_bx_cnt == 1:
        box_orders = [[0]]
    elif init_bx_cnt == 2:
        box_orders = [[0, 1], [1, 0]]
    else:
        box_orders = [list(order) for order in perms]
    ...
    # === 转换为结构化 action（供中间层消费） ===
    # final_p2_state 是推箱结束时的状态（不含基地行走）
    # 用 g_final_p1_state 作为初始状态，前向模拟转换
    p2_initial = SimState(r=g_final_p1_state.r, c=g_final_p1_state.c,
                          mask=g_final_p1_state.mask,
                          b=list(g_final_p1_state.b),
                          bx=list(g_final_p1_state.bx))
    assigned_goal_positions = [255, 255, 255]
    for i in range(init_bx_cnt):
        assigned_goal_positions[i] = dot_pos[cur_assign[i]]
    p2_structured = convert_p2_actions(
        p2_res.path, p2_res.path_length, p2_initial, init_bx_cnt,
        assigned_goal_positions,
    )
    p2_res.structured_actions = p2_structured
