...
class PhaseResult:
    ...
    # === 结构化 action 输出（供中间层消费） ===
    structured_actions: List[dict] = field(default_factory=list)  # 转换后的结构化 action 列表
    observed_box_indices: List[int] = field(default_factory=list)  # P1 中观察到的箱子索引
    observed_target_indices: List[int] = field(default_factory=list)  # P1 中观察到的目标索引
