...
#define CIRCLE_RADIUS_MM           (600.0f)  // 圆轨迹半径
#define CIRCLE_POS_KP              (0.10f)   // 圆轨迹位置误差拉回强度
#define CIRCLE_CORR_LIMIT_RATIO    (0.70f)
#define CIRCLE_SPEED_MAX_RATIO     (1.60f)
...
static void ai_tuner_uart1_rx_handler(void)
{
    uint8 data;
...
#if CIRCLE_MODE_ENABLE
    // 2. 圆圈模式：按里程计闭环跟踪圆轨迹，车头由IMU保持不自转。
