...
#define TEST_UART              (DEBUG_UART_INDEX) // 调试串口UART8：D16发送，D17接收，115200波特率
#define DISPLAY_PERIOD_TICKS   (20U) // 六模式屏幕每20*5ms=100ms刷新一次
#define BOOT_TRACE_ENABLE      (0U) // 1输出上电阶段标记，0关闭，正常调试保持0

// ==================== 用户常用运行配置 ====================
// 总运行模式：0=正式比赛，1=六模式硬件验证，2=点到点串口标定，3=赛道采集串口菜单。
#define CONTROL_DEBUG_ENABLE   (0U)

// 正式比赛界面：0=UART8菜单+C14长按发车+C13长按急停，1=IPS200+C12/C13/C14离线菜单。
// 这个开关只影响CONTROL_DEBUG_ENABLE=0；模式1固定用屏幕，模式2/3固定用串口。
...
#define CONTROL_VERIFY_MODE    (2U)

// 正式比赛动作跟随器的初始化速度，单位为编码器脉冲/5ms；进入每轮后由该轮预设速度接管。
...
static void app_target_init(void)
{
    ...
    uart_write_string(TEST_UART, "#MCU_BOOT id=7100967 stage=CONTROL_DEBUG mode=" STRINGIFY(CONTROL_VERIFY_MODE) " screen=IPS200\r\n");
#elif CONTROL_DEBUG_ENABLE == 2U
    uart_write_string(TEST_UART, "#POINT_TEST_PREBOOT id=7100967 safe=1 menu=UART8_D16_D17 display=OFF telemetry=3s cpr=4096 odom_scale=0.915 pos_type=0x12 car_valid=ON brake=F50-R30-B47-L20 stall=1s assist=300ms\r\n");
#elif CONTROL_DEBUG_ENABLE == 3U
    uart_write_string(TEST_UART, "#TRACK_CAL_PREBOOT id=7100967 safe=1 menu=UART8_D16_D17 display=OFF manual=ON auto=GRID1+GRID2+GRID3+LENGTH36+STANDARD60+FULL180 rotation=AUTO16_45_90_135_180+HAND start_gate=VIS_POS check=5ms approach=20deg/40 brake=7deg stall=1s assist=300ms\r\n");
#else
#if RACE_UI_MODE == RACE_UI_SCREEN
    uart_write_string(TEST_UART, "#OFFLINE_SCREEN_PREBOOT id=7100967 display=IPS200 buttons=C12_C13_C14 uart_menu=OFF\r\n");
#else
    uart_write_string(TEST_UART, "#UART8_D16_D17_RX id=7100967 safe=1 control=UART_BUTTON_HYBRID display=OFF cpr=4096 odom_scale=0.915 pos_type=0x12 car_valid=ON tx=IRQ_QUEUE match=FULL3_PRESET profile=PER_ROUND return=CLEAR_DIRECT+MAP_BFS buttons=C13_HOLD_STOP+C14_HOLD_ARM_START base=ENC_IMU_FORWARD450 wall=SINGLE_1WAY+CORRIDOR_ENC push=STRICT_OBJECT8 heading=POST_MAP_FRAME+IMU_LOCK buzzer=B11_5MS_ISR\r\n");
#endif
#endif
...
static void control_debug_open_loop_task(void)
{
    ...
    ips200_show_string(0, 16, "ID:7100967 PWM:");
...
static void control_debug_show_closed_loop(void)
{
    ...
    if(CONTROL_VERIFY_MODE == 3U)
    {
        ...
        ips200_show_string(0, 16, "ID:7100967 PWM CAP 1800");
    ...
    ips200_show_string(0, 16, "ID:7100967 ZERO=BOOT YAW");
...
int main(void)
{
    ...
    status_buzzer_init();
#if (CONTROL_DEBUG_ENABLE != 1U) && \
    ((CONTROL_DEBUG_ENABLE != 0U) || (RACE_UI_MODE == RACE_UI_UART))
    if(wireless_uart_init() == 0U)
    {
        uart_write_string(TEST_UART, "#WIRELESS_UART8_READY id=7100967 baud=115200 tx=D16 rx=D17 display=OFF\r\n");
    }
    else
    {
        uart_write_string(TEST_UART, "#WIRELESS_UART8_INIT_FAIL id=7100967\r\n");
    }
#endif
    ...
    boot_mark("#BOOT odom");
#if (CONTROL_DEBUG_ENABLE == 1U) || \
    ((CONTROL_DEBUG_ENABLE == 0U) && (RACE_UI_MODE == RACE_UI_SCREEN))
    ...
    roi_camera_link_init();  // Second-camera probe link: MCU UART1 B12/B13.
...
#elif CONTROL_DEBUG_ENABLE == 3U
    point_test_init();       // Reuse the verified encoder+IMU+delayed-vision mover.
    track_calibration_init();// Track acquisition route, records and safety state.
    pc_console_init(3U);     // UART8-only track calibration menu; no display.
#else
    ...
    match_manager_init();    // Three-round/base/return orchestration; starts disarmed.
    action_follower_set_speed(RACE_FREE_MOVE_SPEED);
#if RACE_UI_MODE == RACE_UI_UART
    pc_console_init(0U);     // UART8 menu owns PC commands; motors stay stopped.
    race_button_controls_init(0U); // UART菜单仅启用C13急停和C14准备/发车长按键。
#else
    offline_menu_init();     // IPS200+C12/C13/C14离线菜单，共用比赛配置状态机。
#endif
#endif
    ...
    boot_mark("#BOOT irq");
    status_buzzer_request(STATUS_BUZZER_EVENT_BOOT);
