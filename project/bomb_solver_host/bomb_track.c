...
#define GET_CURRENT_MS() bomb_platform_now_ms()
...
// ==================== 极速静态内存池 ====================
static uint32_t hash_keys[HASH_SIZE]; 
static uint16_t hash_gen[HASH_SIZE] = {0}; 
static uint16_t current_hash_gen = 0;

static struct Node q[MAX_NODES];
static int16_t heap[MAX_NODES]; 
...
static struct QN rq_p2[MAX_P2_Q]; 
...
static uint16_t p2_vis[MAX_R][MAX_C][MAX_R][MAX_C] = {0};
...
static uint16_t global_alive_token[MAX_R][MAX_C][4] = {0};
...
static uint16_t reach_cache_id[MAX_R][MAX_C][4][4] = {0};
static bool reach_cache_val[MAX_R][MAX_C][4][4];
...
bool run_phase1(const char raw_map[12][16], PhaseResult* p1_res, int mode, int skip_flag) {
                ...
                for (int k = start_k; k < lim_k; k++) {
                    ...
                    if (metric >= 0.0f) {
                        ...
                        if (metric < global_best_metric) {
                            global_best_metric = metric; best_p1_steps = temp_p1_len; g_final_p1_state = temp_final_st;
