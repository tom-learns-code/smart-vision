    ...
    for(uint8 i = 0; i < CH_COUNT; i++)
    {
        switch (FW_Data[i].type) 
        {
            ...
            case 'f':
                ...
                break;			
            default:
                uart_write_string(DEBUG_UART_INDEX, "error: Unknown type!\n");
                continue;
        }
        if(i != CH_COUNT-1)
        {
            uart_write_string(DEBUG_UART_INDEX, data_str);
...
void JustFloat_Send()
{
	...
	JF_Data.tail[2] = 0x80;
	JF_Data.tail[3] = 0x7f;
    uart_write_buffer(DEBUG_UART_INDEX, (const uint8*)JF_Data.data, sizeof(float) * CH_COUNT);
    uart_write_buffer(DEBUG_UART_INDEX, (const uint8*)JF_Data.tail, 4);
}
...
{
    uart_write_buffer(DEBUG_UART_INDEX, (const uint8*)preFrame, sizeof(int) * 7);
    uart_write_buffer(DEBUG_UART_INDEX, (uint8 *)image_addr, image_size);
}