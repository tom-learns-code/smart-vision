...
extern int32 hungarian(int32 cost[][MAX_BOXES], int n, int8 assignment[]);
...
static int32 solver_cost[MAX_BOXES][MAX_BOXES];
static push_result_t solver_pushes[MAX_BOXES][MAX_BOXES];
static int8 solver_assign[MAX_BOXES];
                                ...
                                plan_check_result_t *result)
{
    ...
    uint8 active_box[MAX_BOXES];
    uint8 active_goal[MAX_BOXES];
    ...
    memset(active_box, 1, sizeof(active_box));
    memset(active_goal, 1, sizeof(active_goal));
...
solver_status_t solver_solve(map_input_t *map, solver_output_t *out)
{
    ...
    if(solver_mode == SOLVER_MODE_IMAGE_ONLY)
    {
        if(map->bomb_count != 0U ||
           map->box_count > N2_MAX_BOXES ||
           map->goal_count > N2_MAX_BOXES)
        {
