...
#define FOLLOWER_RECOVERY_MAX_DIST_MM       ((float)APP_FOLLOWER_RECOVERY_MAX_MM)
...
static uint8 follower_recover_stable_pose(int16 position_x10,
                                          int16 position_y10)
{
    float dx = (float)follower_debug.target_x_mm -
               (float)position_x10 * 20.0f;
    ...
    float recovery_limit = follower_align_max_distance_mm;

    if(recovery_limit < FOLLOWER_RECOVERY_MAX_DIST_MM)
        recovery_limit = FOLLOWER_RECOVERY_MAX_DIST_MM;
    if(distance > recovery_limit ||
       follower_align_attempts >= follower_align_max_attempts())
        return 0U;

    follower_align_max_distance_mm = recovery_limit;
    follower_debug.align_max_distance_mm =
        (uint16)(recovery_limit + 0.5f);
    return follower_start_alignment(dx, dy);
}

static uint8 follower_accept_wall_constrained_pose(uint8 gx, uint8 gy)
{
    uint8 along_matches = 0U;

    if(follower_debug.strong_reanchor) return 0U;
    if((follower_debug.wall_axis_mask & FOLLOWER_WALL_MASK_X) != 0U &&
       gy == follower_debug.to_y)
        along_matches = 1U;
    if((follower_debug.wall_axis_mask & FOLLOWER_WALL_MASK_Y) != 0U &&
       gx == follower_debug.to_x)
        along_matches = 1U;
    if(!along_matches) return 0U;
                   ...
                   FOLLOWER_POSE_SUSPECT_TICKS)
            {
                ...
                if(gx == follower_debug.to_x && gy == follower_debug.to_y)
                {
                    /* A stable target-cell identity is topologically valid
                       ...
                       Strict/transition nodes still try to return to the
                       ideal center before accepting that identity. */
                    if(!follower_debug.strict_position &&
                       !follower_debug.strong_reanchor)
                        follower_align_attempts =
                            follower_align_max_attempts();
                ...
                {
                    /* The position has now been stable for several frames.
                       Recover to the expected node when the correction is
                       bounded; only a larger or repeatedly failed mismatch
                       is handed to the map replan path. */
                    if(!follower_accept_wall_constrained_pose(gx, gy) &&
                       !follower_recover_stable_pose(
                           follower_suspect_x10, follower_suspect_y10))
                        follower_stop(ACTION_FOLLOWER_REPLAN,
                                      ACTION_FOLLOWER_FAULT_GRID_MISMATCH);
                }
            }
