...
RB = _pt(587, 430)
LB = _pt(33, 429)
TOP_EDGE_POINTS = (LT, _pt(170, 25), _pt(307, 25), _pt(448, 25), RT)
BOTTOM_EDGE_POINTS = (LB, _pt(172, 433), _pt(311, 435), _pt(449, 435), RB)
LEFT_EDGE_POINTS = (LT, _pt(25, 128), _pt(26, 230), _pt(27, 330), LB)
RIGHT_EDGE_POINTS = (RT, _pt(592, 128), _pt(592, 228), _pt(590, 329), RB)
...
LAB_RANGES = {
    "WALL": [
        {"L": (12, 45), "a": (-5, 35), "b": (-30, 8)}
    ],
    "EMPTY": [
        {"L": (18, 42), "a": (45, 88), "b": (-115, -65)}
    ],
    "GOAL": [
        {"L": (35, 68), "a": (65, 110), "b": (-78, -38)}
    ],
    "BOX": [
        {"L": (40, 80), "a": (-8, 28), "b": (42, 88)}
    ],
    "BOMB": [
        {"L": (28, 62), "a": (50, 92), "b": (35, 70)}
    ],
    "PLAYER": [],
}
...
LAB_PLAYER_TRACK = [
    # 车头：青色/蓝绿色
    {"L": (45, 75), "a": (-35, 8), "b": (-55, 5)},
    # 车尾：绿色
    {"L": (45, 75), "a": (-75, -25), "b": (12, 80)},
]
...
# 统一测试开关
DEBUG_TRACK = False
DEBUG_PRINT_FPS = True
DEBUG_PRINT_MAP_ON_FULL = True
DEBUG_PRINT_CAR = True
DEBUG_PRINT_EVENTS = True
DEBUG_PROFILE_TIMING = True
...
sensor.set_auto_exposure(False, exposure_us=200)  # 加这行
    ...
    try:
        for si, th in enumerate(_B_PLAYER_TRACK_SEGS):
            ...
            for blob in blobs:
                ...
                if si < 2:
                    seg_cnt[si] += pixels
                    seg_sum_x[si] += cx * pixels
                    seg_sum_y[si] += cy * pixels
    except Exception as e:
        if DEBUG_PRINT_EVENTS:
            print("EVENT CAR_BLOB_FAIL err=%s" % str(e))
        return None

    if total < PLAYER_BLOB_PIXELS_THRESHOLD:
        return None
...
def print_map_debug(out):
    ...
    bombs = collect_cells(out, ENC["BOMB"], MAX_BOMBS)
    print("==== FULL_MAP debug ====")
    print("boxes", len(boxes), boxes, "goals", len(goals), goals, "bombs", len(bombs), bombs)
    for row in range(GRID_H):
        print(map_row_text(out, row))

def print_car_debug(fps_value, car_x10, car_y10, car_theta_x10, car_valid):
    if car_valid:
        print("FPS %.1f CAR (%.1f,%.1f) theta=%.1f deg" %
              (fps_value, car_x10 / 10.0, car_y10 / 10.0, car_theta_x10 / 10.0))
    else:
        print("FPS %.1f CAR lost" % fps_value)
...
while True:
    ...
    need_map_refresh = (not map_cache_ready) or need_full_map or \
        time.ticks_diff(now_ms, last_full_map_refresh_ms) >= FULL_MAP_REFRESH_PERIOD_MS
    ...
    if need_map_refresh:
        if DEBUG_PRINT_EVENTS:
            if need_full_map:
                ...
                print("EVENT MAP_REFRESH start reason=init")
        else:
            print("EVENT MAP_REFRESH start reason=periodic")
    ...
    if pos is not None:
        player_cx, player_cy, head_cx, head_cy, tail_cx, tail_cy = pos
        player_found = True
        last_car_x10, last_car_y10, last_player_gx, last_player_gy = \
            local_player_grid_values(player_cx, player_cy)
    ...
    if need_full_map or need_map_refresh:
        profile_t0 = time.ticks_ms()
        send_full_map(out, last_car_x_mm, last_car_y_mm, last_car_theta_x10)
        profile_send_ms += time.ticks_diff(time.ticks_ms(), profile_t0)
        if need_full_map and DEBUG_PRINT_MAP_ON_FULL:
            print_map_debug(out)
        full_map_requested = False
    ...
    if DEBUG_PRINT_FPS and time.ticks_diff(now_ms, last_debug_ms) >= DEBUG_FPS_PERIOD_MS:
        ...
        if DEBUG_PRINT_CAR:
            print_car_debug(fps_value, last_car_x10, last_car_y10, last_car_theta_x10, car_known)
        else:
