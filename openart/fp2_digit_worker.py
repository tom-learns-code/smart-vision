...
MODEL = "/sd/smartcar_digit_model_qat.tflite"
ROI = (55, 40, 245, 180)
THRESHOLD = 0.80
SAMPLE_COUNT = 3
            ...
            for result in tf.classify(net, img, min_scale=1.0, scale_mul=0.5,
                ...
                if scores:
                    for candidate in range(len(scores)):
                        score_sums[candidate] += scores[candidate]
                    valid_samples += 1
                    break
            del img
        if valid_samples == 0:
            raise RuntimeError("no valid inference samples")
        class_id, confidence_sum = highest(score_sums)
        confidence = confidence_sum / valid_samples
        raw_id = class_id
        status = 0 if class_id != 0xFF and confidence >= THRESHOLD else 1
    except Exception:
        status = 7
    finally:
        if net is not None:
