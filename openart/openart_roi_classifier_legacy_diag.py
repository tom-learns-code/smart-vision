...
IDE_DEBUG_LEGACY_MODEL_ONLY = True
...
def print_storage_diagnostics():
    if not IDE_DEBUG_ENABLE:
        return
    print("FP_FS_DIAG begin")
    for root in (".", "/", "/sd", "/flash", "/sdcard"):
        try:
            entries = os.listdir(root)
            print("FP_FS root=%s count=%d entries=%s" % (root, len(entries), entries))
        except Exception as error:
            print("FP_FS root=%s unavailable err=%s" % (root, error))
    print("FP_FS_DIAG end")
    ...
    try:
        ...
        if MODEL_LOAD_TO_FB:
            try:
                ...
                print("FP %s fb load failed, retry file: %s" % (model_name, error))
                gc.collect()
...
def classify_image_model_file(candidates, model_name, img):
    ...
    print(
        "FP_IMAGE_STAGE model=%s phase=INFER_BEGIN load=%dms"
        % (model_name, load_ms)
    )
    ...
    # Release each model before loading the next one so only one large model
    # and its tensor arena are resident at a time.
    del net
    gc.collect()
    total_ms = time.ticks_diff(time.ticks_ms(), stage_started)
    print(
        "FP_IMAGE_STAGE model=%s phase=RELEASED id=%d conf=%.4f status=%d load=%dms infer=%dms total=%dms"
        % (
            model_name, result[0], result[1], result[2],
            load_ms, inference_ms, total_ms,
        )
    )
    return result, total_ms


def load_digit_model_if_needed():
    global digit_net
    global digit_labels
    global digit_model_attempted

    if digit_model_attempted:
        return digit_net is not None
    ...
    try:
        ...
        if not digit_labels:
            ...
            return False
        digit_net = load_model_file(DIGIT_MODEL_CANDIDATES, "digit")
        print(
            "FP digit model ready net=%d labels=%d"
            % (1 if digit_net is not None else 0, len(digit_labels))
        )
        return digit_net is not None
    except Exception as error:
        print("FP digit model setup failed: %s" % error)
        digit_net = None
        return False
...
def classify_image_ensemble(img):
    ...
    if not load_image_models_if_needed():
        ...
        result_1, model_1_ms = classify_image_model_file(
            IMAGE_MODEL_1_CANDIDATES, "image1", img
        )
        result_2, model_2_ms = classify_image_model_file(
            IMAGE_MODEL_2_CANDIDATES, "image2", img
        )
...
def classify_ide_image(img):
    if not IDE_DEBUG_LEGACY_MODEL_ONLY:
        return classify_image_ensemble(img)
    if not load_image_models_if_needed():
        return 0xFF, 0.0, FP_STATUS_MODEL_UNAVAILABLE
    result, total_ms = classify_image_model_file(
        LEGACY_IMAGE_MODEL_CANDIDATES, "legacy", img
    )
    print(
...
def classify_digit_once(img):
    started = time.ticks_ms()
    if not load_digit_model_if_needed():
        return 0xFF, 0.0, FP_STATUS_MODEL_UNAVAILABLE
    class_index, confidence, status = classify_loaded_model(digit_net, img)
    if class_index != 0xFF:
    ...
    print(
        "FP_DIGIT model_class=%d label=%s id=%d conf=%.4f status=%d total=%dms"
        % (
            class_index, label_name(digit_labels, class_index), result_id,
            confidence, status, elapsed_ms,
        )
    )
    return result_id, confidence, status
...
def process_pending_request(img):
    global pending_request
    ...
    if FAKE_RECOGNITION_ENABLE:
        ...
        raw_id, confidence, status = classify_image_ensemble(img)
        ...
        raw_id, confidence, status = classify_digit_once(img)
        result_id, confidence, status = compact_real_result(
            raw_id, confidence, status
        )
        send_result(session_id, request_id, object_type, object_slot,
...
def process_ide_debug(img, request_processed):
    ...
    if IDE_DEBUG_MODE == IDE_DEBUG_MODE_IMAGE or IDE_DEBUG_MODE == IDE_DEBUG_MODE_BOTH:
        result_id, confidence, status = classify_ide_image(img)
        print(
    ...
    if IDE_DEBUG_MODE == IDE_DEBUG_MODE_DIGIT or IDE_DEBUG_MODE == IDE_DEBUG_MODE_BOTH:
        result_id, confidence, status = classify_digit_once(img)
        print(
...
while True:
    ...
    request_processed = process_pending_request(frame)
    process_ide_debug(frame, request_processed)
